...
export class Fat32FileSystem : public FileSystemDriver {
  ...
  public:
    ...
    explicit Fat32FileSystem(Device* device);
...
Fat32FileSystem::Fat32FileSystem(Device* device) : device(device) {}

void Fat32FileSystem::parse_filename(StringView path, u8* dest_name, u8* dest_ext) {
...
void Fat32FileSystem::format_name(const u8* raw_name, const u8* raw_ext,
                                         char* dest_node_name) {
    usize index = 0;
    usize name_len = 8;
...
bool Fat32FileSystem::find_dir_entry(StringView name, FatDirEntry& out_entry,
                                            usize& out_sector, usize& out_offset) {
    if (device == nullptr) {
        return false;
    }
    ...
    while (current_cluster < 0x0FFFFFF8) {
        for (usize sector_offset = 0; sector_offset < sectors_per_cluster; ++sector_offset) {
            usize current_sector = data_start_sector + (current_cluster - 2) * sectors_per_cluster + sector_offset;

            if (device->read(current_sector * bytes_per_sector, sector_buffer, bytes_per_sector) !=
                bytes_per_sector) {
                return false;
            }
...
u32 Fat32FileSystem::read_fat_entry(u32 cluster) {
    if (device == nullptr) {
        return 0x0FFFFFFF;
    }
...
usize Fat32FileSystem::read_file(StringView path, usize offset, u8* buffer, usize size) {
    StringView filename = path;
    ...
    u32 current_cluster = (static_cast<u32>(entry.first_cluster_high) << 16) | entry.first_cluster_low;
    ...
    while (bytes_read < bytes_to_read && current_cluster < 0x0FFFFFF8) {
        ...
        if (device->read(sector * bytes_per_sector + start_offset_in_cluster, buffer + bytes_read, read_len) !=
            read_len) {
            break;
        }
...
bool Fat32FileSystem::create_file(StringView path) {
    if (device == nullptr) {
        return false;
    }
        ...
        for (usize sector_offset = 0; sector_offset < sectors_per_cluster; ++sector_offset) {
            ...
            for (usize i = 0; i < entries_per_sector; ++i) {
                if (entries[i].name[0] == 0x00 || entries[i].name[0] == 0xE5) {
                    ...
                    entries[i].first_cluster_high = static_cast<u16>((free_cluster >> 16) & 0xFFFF);
...
bool Fat32FileSystem::delete_file(StringView path) {
    if (device == nullptr) {
        return false;
    }
    ...
    u32 curr_cluster = (static_cast<u32>(entry.first_cluster_high) << 16) | entry.first_cluster_low;
    ...
    if (device->read(dir_sector * bytes_per_sector, sector_buffer, bytes_per_sector) == bytes_per_sector) {
        auto* raw_entry = reinterpret_cast<FatDirEntry*>(&sector_buffer[dir_offset]);
        raw_entry->name[0] = 0xE5;
        return device->write(dir_sector * bytes_per_sector, sector_buffer, bytes_per_sector) == bytes_per_sector;
    }

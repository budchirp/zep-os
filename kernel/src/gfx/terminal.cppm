    ...
    void draw_glyph(u64 x, u64 y, char c) {
        ...
        for (u64 row = 0; row < glyph_height; ++row) {
            ...
            for (u64 col = 0; col < glyph_width; ++col) {
                ...
                framebuffer.write(x + col, y + row, pixel_on ? fg_color : bg_color);
            }
        }
    }
            ...
            for (u64 x = 0; x < columns; ++x) {
                for (u64 py = 0; py < glyph_height; ++py) {
                    for (u64 px = 0; px < glyph_width; ++px) {
                        framebuffer.write(x * glyph_width + px, y * glyph_height + py, bg_color);
                    }
                }
            }
        }
  ...
  public:
    ...
    explicit Terminal(Framebuffer& framebuffer)
        : framebuffer(framebuffer), cursor_x(0), cursor_y(0),
          ...
          fg_color(0x000000), bg_color(0xFFFFFF) {}
...
extern "C" void zep_terminal_clear(Terminal* terminal) {
    terminal->clear();
}

extern "C" void zep_terminal_print(Terminal* terminal, string str) {
    terminal->print(str);
}
